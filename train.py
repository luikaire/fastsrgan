...
def main(config):
    ...
    if not os.path.exists(config.data.numpy_dir_lq):
        write_images_to_numpy_arrays(
            [
                os.path.join(config.data.image_dir_lq, x)
                for x in os.listdir(config.data.image_dir_lq)
                if x.endswith(".png")
            ],
            config.data.numpy_dir_lq,
        )
    g = torch.Generator()
    ...
    numpy_files_lq = [
        os.path.join(config.data.numpy_dir_lq, x)
        for x in os.listdir(config.data.numpy_dir_lq)
        if x.endswith(".npy")
    ]
    train_dataset = NumpyImagesDataset(
        numpy_files,numpy_files_lq, config.data.lr_image_size, config.data.scale_factor
    )
